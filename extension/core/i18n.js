...
const ZH = {
  ...
  'ui.skipNavigated': '已跳过 {name}：页面已跳转，编号需重新获取',
  // 过程时间轴折叠后的一行摘要：步数 + 可选的页面操作数与失败数，按顺序拼接
  'ui.traceStep': '已执行 1 步',
  'ui.traceSteps': '已执行 {n} 步',
  'ui.traceAction': '，含 1 次页面操作',
  'ui.traceActions': '，含 {n} 次页面操作',
  'ui.traceFailed': '，{n} 步失败',
  'ui.traceToggle': '展开 / 折叠过程',

  /* ---------- 界面：历史会话 ---------- */
  'ui.history': '历史会话',
  'ui.historyEmpty': '还没有历史会话。发出第一条消息后，会话会自动保存到这里。',
  'ui.historyCurrent': '当前会话',
  'ui.historyTurn': '1 轮对话',
  ...
  'ui.menuCompactUsage': '/compact · 已用约 {pct}%',
  'ui.ctxWarn': '上下文已用约 {pct}%（约 {used} / {total} tokens）。建议现在压缩——等到请求超窗报错，压缩也会失败。',
  'ui.ctxHigh': '上下文已用约 {pct}%（约 {used} / {total} tokens），快到模型上限了。请立即压缩，或点「新对话」重新开始。',
  'ui.ctxOver': '按估算，上下文已超出模型窗口（约 {used} / {total} tokens），下一次请求多半会失败。请立即压缩，或点「新对话」重新开始。',
  'ui.ctxCompactNow': '压缩',
  ...
  'ui.cfgContextWindowHint': '用于估算上下文用量，快满时提醒压缩。可写 128k；留空按 64k 估算。',
  'ui.cfgActions': '允许页面操作',
  'ui.cfgTest': '测试连接',
  'ui.cfgSave': '保存',
  'ui.cfgProfiles': '模型接口',
  'ui.cfgProfilesHint': '下拉里选中的即为当前使用的一套；增删改都在点「保存」后生效。',
  'ui.cfgProfileAdd': '新增一套接口',
  'ui.cfgProfileDelete': '删除这套接口',
  'ui.cfgProfileUnnamed': '未命名',
  'ui.cfgProfileRemoved': '已移除，保存后生效。',
  'ui.cfgProfileName': '名称',
  'ui.cfgProfileNamePlaceholder': '如 DeepSeek 官方、行内部署',
  'ui.cfgPrefs': '偏好',
  'ui.cfgTransfer': '备份与恢复',
  'ui.cfgExport': '导出设置',
  'ui.cfgImport': '导入设置',
  'ui.exportOk': '已导出当前已保存的设置。',
  'ui.importConfirm': '导入将覆盖现有的全部接口配置与偏好，继续？',
  'ui.importOk': '已导入 {n} 套接口配置。',
  'ui.importBad': '不是有效的 Titanium 设置文件。',
  'ui.importReadFail': '读取文件失败，请重试。',
  'ui.testRunning': '正在测试…',
  ...
  'err.contextOverflow': '请求超出了模型的上下文窗口。可以先试「压缩上下文」；若压缩同样报这个错，只能点「新对话」重新开始。',
  'err.contextOverflowCompact': '压缩请求本身也超出了模型的上下文窗口，此前的对话已无法压缩，请点「新对话」重新开始。',
  'err.detailPrefix': '：',
  ...
  'act.wait.run': '正在等待页面加载…',
  'act.wait.fail': '等待页面加载失败',
  'act.wait.done': '已等待 {s} 秒{settled}',
  'act.wait.settled': '，页面已稳定',
  'act.wait.unsettled': '，页面仍在变动',
  'act.click.run': '正在点击元素 [{ref}]…',
  'act.click.fail': '点击元素 [{ref}] 失败',
  'act.click.done': '已点击 [{ref}]{name}{jumped}',
  'act.input.run': '正在向 [{ref}] 输入「{preview}」…',
  'act.input.fail': '向 [{ref}] 输入失败',
  'act.input.done': '已向 [{ref}]{name} 输入「{preview}」{jumped}',
  ...
  'act.back.run': '正在后退…',
  'act.back.fail': '后退失败',
  'act.back.done': '已后退到：{title}',
  'act.refresh.run': '正在刷新页面…',
  'act.refresh.fail': '刷新失败',
  'act.refresh.done': '已刷新：{title}',
  'act.openTab.run': '正在新标签页打开 {url}…',
  'act.openTab.fail': '新标签页打开失败',
  'act.openTab.done': '已在新标签页打开：{title}',
  'act.switchTab.run': '正在切换到标签页 {id}…',
  'act.switchTab.fail': '切换标签页失败',
  'act.switchTab.done': '已切换到：{title}',
  'act.closeTab.run': '正在关闭标签页…',
  'act.closeTab.fail': '关闭标签页失败',
  'act.closeTab.done': '已关闭标签页',
  ...
  'fmt.searchHeadOne': '共找到 1 处「{query}」（@ 后是该处在完整正文中的字符位置，可交给 read_page_text 读上下文）：',
  ...
  'res.waited': '已等待 {s} 秒{settled}。',
  'res.waitedSettled': '，页面内容已稳定',
...
const EN = {
  ...
  'ui.skipNavigated': 'Skipped {name}: the page navigated, element numbers must be fetched again',
  // 过程时间轴折叠后的一行摘要：步数 + 可选的页面操作数与失败数，按顺序拼接
  'ui.traceStep': 'Ran 1 step',
  'ui.traceSteps': 'Ran {n} steps',
  'ui.traceAction': ', 1 page action',
  'ui.traceActions': ', {n} page actions',
  'ui.traceFailed': ', {n} failed',
  'ui.traceToggle': 'Expand / collapse the steps',

  /* ---------- UI: history ---------- */
  'ui.history': 'History',
  'ui.historyEmpty': 'No conversations yet. They are saved here automatically once you send a message.',
  'ui.historyCurrent': 'Current',
  'ui.historyTurn': '1 turn',
  ...
  'ui.menuCompactHint': '/compact',
  'ui.menuCompactUsage': '/compact · ~{pct}% used',
  'ui.ctxWarn': 'Context is about {pct}% full (~{used} / {total} tokens). Compact now: once requests overflow the window, compaction fails too.',
  'ui.ctxHigh': 'Context is about {pct}% full (~{used} / {total} tokens) and close to the model limit. Compact now, or start a New chat.',
  'ui.ctxOver': 'By estimate the context already exceeds the model window (~{used} / {total} tokens), so the next request will probably fail. Compact now, or start a New chat.',
  'ui.ctxCompactNow': 'Compact',
  ...
  'ui.cfgContextWindowHint': 'Used to estimate context usage and remind you to compact before it fills up. 128k works; blank means 64k.',
  'ui.cfgActions': 'Allow page actions',
  'ui.cfgTest': 'Test connection',
  'ui.cfgSave': 'Save',
  'ui.cfgProfiles': 'Model endpoints',
  'ui.cfgProfilesHint': 'The one selected here is the one in use; additions, edits and removals apply when you save.',
  'ui.cfgProfileAdd': 'Add an endpoint',
  'ui.cfgProfileDelete': 'Remove this endpoint',
  'ui.cfgProfileUnnamed': 'Untitled',
  'ui.cfgProfileRemoved': 'Removed; applies when you save.',
  'ui.cfgProfileName': 'Name',
  'ui.cfgProfileNamePlaceholder': 'e.g. DeepSeek, in-house deployment',
  'ui.cfgPrefs': 'Preferences',
  'ui.cfgTransfer': 'Backup & restore',
  'ui.cfgExport': 'Export settings',
  'ui.cfgImport': 'Import settings',
  'ui.exportOk': 'Saved settings exported.',
  'ui.importConfirm': 'Importing replaces all current endpoints and preferences. Continue?',
  'ui.importOk': 'Imported {n} endpoint(s).',
  'ui.importBad': 'Not a valid Titanium settings file.',
  'ui.importReadFail': 'Could not read the file, please try again.',
  ...
  'err.contextOverflow': 'The request exceeds the model\'s context window. Try Compact context first; if that fails with the same error, only New chat is left.',
  'err.contextOverflowCompact': 'The compaction request itself exceeds the model\'s context window, so this conversation can no longer be compacted. Please start a New chat.',
  'err.detailPrefix': ': ',
  ...
  'act.find.one': 'Searched "{query}": 1 match',
  ...
  'act.wait.run': 'Waiting for the page to load…',
  'act.wait.fail': 'Waiting for the page failed',
  'act.wait.done': 'Waited {s}s{settled}',
  'act.wait.settled': ', page settled',
  'act.wait.unsettled': ', page still changing',
  'act.click.run': 'Clicking element [{ref}]…',
  'act.click.fail': 'Failed to click element [{ref}]',
  'act.click.done': 'Clicked [{ref}]{name}{jumped}',
  'act.input.run': 'Typing "{preview}" into [{ref}]…',
  'act.input.fail': 'Failed to type into [{ref}]',
  'act.input.done': 'Typed "{preview}" into [{ref}]{name}{jumped}',
  ...
  'act.back.run': 'Going back…',
  'act.back.fail': 'Going back failed',
  'act.back.done': 'Went back to: {title}',
  'act.refresh.run': 'Reloading the page…',
  'act.refresh.fail': 'Reload failed',
  'act.refresh.done': 'Reloaded: {title}',
  'act.openTab.run': 'Opening {url} in a new tab…',
  'act.openTab.fail': 'Failed to open a new tab',
  'act.openTab.done': 'Opened in a new tab: {title}',
  'act.switchTab.run': 'Switching to tab {id}…',
  'act.switchTab.fail': 'Failed to switch tabs',
  'act.switchTab.done': 'Switched to: {title}',
  'act.closeTab.run': 'Closing the tab…',
  'act.closeTab.fail': 'Failed to close the tab',
  'act.closeTab.done': 'Closed the tab',
  ...
  'fmt.searchHeadOne': 'Found 1 occurrence of "{query}" (the number after @ is the character position in the complete body text — pass it to read_page_text to read around it):',
  ...
  'res.waited': 'Waited {s}s{settled}.',
  'res.waitedSettled': '; the page content has settled',
